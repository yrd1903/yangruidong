...
*/
std::string FastText::predict(std::istream& in, int32_t k, bool print_prob) {
	std::vector<std::pair<real, std::string>> predictions;
	std::ostringstream res;//存放返回结果
	while (in.peek() != EOF) {//这里的循环是检测文件中每一行
		predict(in, k, predictions);
		if (predictions.empty()) {
			res << std::endl;
			continue;
