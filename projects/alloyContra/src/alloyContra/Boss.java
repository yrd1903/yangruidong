...
public class Boss {
	...
	private Image[] boss_attack=new Image[6];{                       //bossµÄÍ¼Æ¬ 
		for(int i=0;i<6;i++){
			boss_attack[i]=kit.getImage("img\\boss_attack"+i+".png");
		}
	}
	private Image[] bomb_bang=new Image[18];{                      //bossµÄÕ¨µ¯±¬Õ¨Í¼Æ¬
		for(int i=0;i<18;i++){
			bomb_bang[i]=kit.getImage("img\\bomb_bang"+i+".png");
		}
	}
	private Image boss_bomb=Toolkit.getDefaultToolkit().getImage("img\\boss_bomb.png");         //bossµÄÕ¨µ¯
	
	private Image[] plane_bomb=new Image[5];{                   //·É»úÕ¨µ¯µÄÍ¼Æ¬
		for(int i=0;i<5;i++){
			plane_bomb[i]=kit.getImage("img\\plane_bomb"+i+".png");
		}
	}
	private Image[] plane_fly=new Image[5];{                        //·É»úµÄÍ¼Æ¬
		for(int i=0;i<5;i++){
			plane_fly[i]=kit.getImage("img\\plane_fly"+i+".png");
		}
	}
	private Image[] tank_hit=new Image[20];{                           //Ì¹¿Ë±»»÷ÖÐÍ¼Æ¬
		for(int i=0;i<20;i++){
			tank_hit[i]=kit.getImage("img\\tank_hit"+i+".png");
		}
	}
	private Image[] D_boss=new Image[19];{                        //Ì¹¿Ë±»»÷»ÙºóµÄbossÍ¼Æ¬
		for(int i=0;i<19;i++){
			D_boss[i]=kit.getImage("img\\D_boss ("+i+").png");
		}
	}
	private Image[] lost=new Image[9];{                     //boss Í¶½µÍ¼Æ¬
		for(int i=0;i<9;i++){
			lost[i]=kit.getImage("img\\lost"+i+".png");
		}
	}
	private Image[] monster=new Image[24];{          //ÖÕ¼«´óbossÍ¼Æ¬
		for(int i=0;i<24;i++){ 
			monster[i]=kit.getImage("img\\monster ("+i+").png");
		}
	}
	private Image[] fire=new Image[10];{              //»ðÑæÍ¼Æ¬
		for(int i=0;i<10;i++){
			fire[i]=kit.getImage("img\\fire"+i+".png");
		}
	}
	private Image[] monster_die=new Image[22];{            //´óbossËÀµÄÍ¼Æ¬
		for(int i=0;i<22;i++){
		...
		if(bomb_y<350){
			...
			if(bang_type<17){
				if(ci_bang<20){
					...
					//±¬Õ¨ÉùÒô
					if(bang_type==0&&P>0&&P<1000){
						Background.music.mc_boss_bomb();
					}
					ci_bang=0;
					bang_type++;
				}
			...
			if(move_bomb_x<400){
				...
				if(bomb_bang_type<17){
					if(ci_bang<30){
						...
						bomb_bang_type++;
						//±¬Õ¨ÉùÒô
						if(bomb_bang_type==1&&boss_bomb_position_x>0&&boss_bomb_position_x<1000){
							Background.music.mc_boss_bomb();
						}
					}
	...
	public void die_monster(Graphics g){
		...
		if(monster_die_type<21){
			...
			boss_number=0;
			//½áÊøÒôÀÖ
			Background.music.mc_over();
			
			int i=JOptionPane.showOptionDialog(b,Background.h, 
